...
```
The data frame **activity** has **`r x`** rows and **`r y`** columns
...
```
`r paste("Mean total number of steps taken per day:", meantotsteps)`
...
```
`r paste("Median total number of steps taken per day:", medtotsteps)`
...
```
The 5-minute interval with the maximum number of average steps is interval number **`r intname`** with **`r maxsteps`** steps.
...
The total number of rows with NAs is **`r totnas`**
